...
<div class="min-h-screen bg-slate-50">
    <div class="bg-white/80 border-b border-slate-200 backdrop-blur">
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
            <h1 class="text-lg font-bold text-slate-900">Create quiz</h1>
            <a href="{{ route('teacher.quizzes.index') }}" class="text-sm text-blue-600 font-medium">Back</a>
        </div>
    </div>
    <main class="max-w-xl mx-auto px-6 py-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <form method="POST" action="{{ route('teacher.qams.quizzes.store') }}" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                </div>
                <div>
                ...
                <p class="text-xs text-slate-500">Students will answer from your question bank for this subject.</p>
