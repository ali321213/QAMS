...
<div class="min-h-screen bg-slate-50">
    <div class="bg-white/80 border-b border-slate-200 backdrop-blur">
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
            <h1 class="text-lg font-bold text-slate-900">Register student</h1>
    ...
    <main class="max-w-2xl mx-auto px-6 py-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <form method="POST" action="{{ route('admin.qams.students.store') }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-medium text-slate-600 mb-1">Full name</label>
                    <input type="text" name="name" value="{{ old('name') }}" required maxlength="30" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 mb-1">Username</label>
                    <input type="text" name="user_name" value="{{ old('user_name') }}" required maxlength="30" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="admission_number" value="{{ old('admission_number') }}" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 mb-1">Father's name</label>
                    <input type="text" name="father_name" value="{{ old('father_name') }}" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                </div>
                <div>
                    ...
                    <div class="max-h-48 overflow-y-auto border border-slate-200 rounded-lg p-3 space-y-2">
                        @foreach ($subjects as $sub)
                            <label class="flex items-center gap-2 text-sm">
                                <input type="checkbox" name="subject_ids[]" value="{{ $sub->id }}" @checked(in_array($sub->id, old('subject_ids', [])))>
                                <span>{{ $sub->schoolClass->name }} — {{ $sub->name }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Register</button>
            </form>
        </div>
    </main>
</div>
