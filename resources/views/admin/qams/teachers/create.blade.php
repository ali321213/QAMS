...
<div class="min-h-screen bg-slate-50">
    <div class="bg-white/80 border-b border-slate-200 backdrop-blur">
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
            <h1 class="text-lg font-bold text-slate-900">Register teacher</h1>
    ...
    <main class="max-w-2xl mx-auto px-6 py-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <form method="POST" action="{{ route('admin.qams.teachers.store') }}" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="user_name" value="{{ old('user_name') }}" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="job_history" rows="3" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">{{ old('job_history') }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 mb-1">Education</label>
                    <textarea name="education" rows="3" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">{{ old('education') }}</textarea>
                </div>
                <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Register</button>
            </form>
        </div>
    </main>
</div>
